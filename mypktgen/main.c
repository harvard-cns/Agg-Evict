  ...
  /*  if (unlikely((open == 0)&&(ret == 0))){
    ...
    */
    ...
    if(ginfo.total_trace * pkt_times <= sended)
    {
    	...
    	printf("Done. pkt number = %u, sent pkt number = %u, time used =　%lfs\n\n", ginfo.total_trace, sended, time_consume);
    	printf("\nEND trace_idx = %u, droped = %u/%u = %lf, lcoreid = %u\n", largs->trace_idx, droped, sended, droped * 1.0 / sended, rte_lcore_id());
    	rte_exit(0, "Completed\n");
        // rte_eal_intr_exit();
        // pthread_exit(0);
    }
    rte_spinlock_unlock(&sended_spinlock);
...
static int sender_lcore_main(void *args)
{
   ...
   long int ii = 0;
    for(;;) {
        ...
        {
            rte_timer_manage(); 
            if(cmd_change_rate == 1)
            {
                largs->speed = ginfo.Mbps * 1024 * 1024 / NB_TXQ;
                uint64_t period = calc_period(largs->speed);
                rte_timer_reset(&largs->tx.tim, period, PERIODICAL, rte_lcore_id(), send_pkt_rate, largs);
            }
        }
...
static void print_stats(int nb_ports, int print_num)
{
    int i, j;
    uint64_t tx_total;
...
// the "1" added for count is specified for master lcore. Each slave lcore works for only one queue (TXQ or RXQ). Each port is assigned with NB_TXQ HW ring(s) for transmitting and with NB_RXQ HW ring(s) for receiving.  
    ...
    {  
        rte_exit(0,"Failed to set address reuse.\n");  
    }  
    ...
    uint64_t last_miss = -1;
    uint64_t now_miss = -1;

    uint64_t up_rate = 10000;
    uint64_t down_rate = 200;


#define AUTO

    // FILE * rate_file = fopen("sending_rate.txt", "a");


#ifdef AUTO
    ginfo.Mbps = (up_rate + down_rate) / 2.0;
    int print_num = 2;
#else
    int print_num = 1 << 30;
    // fprintf(rate_file, "stable sending rate = %lf pps\n", ginfo.Mbps / 10000.0 * 14880000.0);
#endif
    ...
    while(1)
    {
        ...
        print_stats(nb_ports, print_num);


        recv( client_sockfd, buf, sizeof(buf), MSG_DONTWAIT);
        while((len=recv(client_sockfd,&now_miss,8,0))>0)
        {  
            printf("%lu vs. %lu\n",now_miss, last_miss);
        ...
        if(last_miss != -1)
        {
            if(now_miss - last_miss <= 1024)
            {
                ...
                {
                    down_rate = ginfo.Mbps;
                    ginfo.Mbps = (down_rate + up_rate) / 2.0;
                }
            }
            else
            {
