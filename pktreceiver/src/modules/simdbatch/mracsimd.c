...
inline static uint32_t mracsimd_size(MRACSimdPtr ptr) 
{
    return ptr->size * 4;
}

inline void mracsimd_reset(MRACSimdPtr ptr) 
{
    ...
    for(int i = 0; i < SIMD_ROW_NUM; i++)
    {
        ...
        for(int j = 0; j < 16; j++)
        {
            mracsimd_inc_mr(ptr, (const void *)(p_key_bucket + j), *(p_value_bucket + j));
        }
    }

    memset(ptr->table, 0, mracsimd_size(ptr) * (ptr->elsize) * sizeof(uint32_t));
...
MRACSimdPtr mracsimd_create(uint32_t size, uint16_t keysize, uint16_t elsize, int socket) 
{
    ...
    MRACSimdPtr ptr = rte_zmalloc_socket(
        0, 
        sizeof(struct MRACSimd) + 
...
inline int mracsimd_inc_mr(MRACSimdPtr ptr, void const * kick_key, uint32_t kick_val)
{
    uint32_t hv = dss_hash_with_seed(kick_key, ptr->keysize, CMS_SH1) & (ptr->M - 1);
...
inline int mracsimd_inc(MRACSimdPtr ptr, void const *key) 
{
    ...
    uint32_t ret = mracsimd_inc_mr(ptr, (const void *)(p_key_bucket + curkick), p_value_bucket[curkick]);
    ...
    uint32_t ret = mracsimd_inc_mr(ptr, (const void *)(p_key_bucket + kick_index), p_value_bucket[kick_index]);
...
inline void mracsimd_delete(MRACSimdPtr ptr) 
{
    rte_free(ptr->simd_ptr);
    rte_free(ptr);
}

inline uint32_t mracsimd_num_searches(MRACSimdPtr ptr)
{ 
...
ModulePtr simdbatch_mracsimd_init(ModuleConfigPtr conf) 
{
    ...
    ModuleSimdBatchMRACSimdPtr module = rte_zmalloc_socket(0,
            sizeof(struct ModuleSimdBatchMRACSimd), 64, socket); 

    module->_m.execute = simdbatch_mracsimd_execute;
    module->size  = size;
    module->keysize = keysize;
    module->elsize = elsize;
    module->socket = socket;

    module->mracsimd_ptr1 = mracsimd_create(size, keysize, elsize, socket);
    module->mracsimd_ptr2 = mracsimd_create(size, keysize, elsize, socket);

    module->mracsimd = module->mracsimd_ptr1;

    return (ModulePtr)module;
}

void simdbatch_mracsimd_delete(ModulePtr module_) 
{
    ModuleSimdBatchMRACSimdPtr module = (ModuleSimdBatchMRACSimdPtr)module_;

    mracsimd_delete(module->mracsimd_ptr1);
    mracsimd_delete(module->mracsimd_ptr2);

    rte_free(module);
}

inline void simdbatch_mracsimd_execute(
        ModulePtr module_,
        ...
        uint32_t count) 
{
    ...
    MRACSimdPtr mracsimd = module->mracsimd;

    /* Prefetch mracsimd entries */
    for (i = 0; i < count; ++i) 
    {
        uint8_t const* pkt = rte_pktmbuf_mtod(pkts[i], uint8_t const*);
        mracsimd_inc(mracsimd, (pkt + 26));
    }
}

inline void simdbatch_mracsimd_reset(ModulePtr module_) 
{
    ModuleSimdBatchMRACSimdPtr module = (ModuleSimdBatchMRACSimdPtr)module_;
    MRACSimdPtr prev = module->mracsimd;

    if (module->mracsimd == module->mracsimd_ptr1) 
    {
        module->mracsimd = module->mracsimd_ptr2;
    } 
    else 
    {
        module->mracsimd = module->mracsimd_ptr1;
    ...
    module->stats_search += mracsimd_num_searches(prev);
    mracsimd_reset(prev);
}

inline void simdbatch_mracsimd_stats(ModulePtr module_, FILE *f) 
{
    ModuleSimdBatchMRACSimdPtr module = (ModuleSimdBatchMRACSimdPtr)module_;
    module->stats_search += mracsimd_num_searches(module->mracsimd);
    fprintf(f, "SimdBatch::MRACSimd::SearchLoad\t%u\n", module->stats_search);
}
