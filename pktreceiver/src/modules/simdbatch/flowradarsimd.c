...
#include "flowradarsimd.h"




inline static uint32_t flowradarsimd_size(FlowRadarSimdPtr ptr) 
{
    return ptr->size * (sizeof(struct Cell) + 4);
}

inline void flowradarsimd_reset(FlowRadarSimdPtr ptr) 
{
    ...
    for(int i = 0; i < SIMD_ROW_NUM; i++)
    {
        ...
        for(int j = 0; j < 16; j++)
        {
            flowradarsimd_inc_fr(ptr, (const void *)(p_key_bucket + j), *(p_value_bucket + j));
        }
    }

    memset(ptr->table, 0, flowradarsimd_size(ptr));
...
FlowRadarSimdPtr flowradarsimd_create(uint32_t size, uint16_t keysize, uint16_t elsize, int socket) 
{
    ...
    FlowRadarSimdPtr ptr = rte_zmalloc_socket(
        0, 
        sizeof(struct FlowRadarSimd) + 
...
inline int  flowradarsimd_inc_fr(FlowRadarSimdPtr ptr, void const * kick_key, uint32_t kick_val)
{
...
inline int flowradarsimd_inc(FlowRadarSimdPtr ptr, void const *key) 
{
    ...
    uint32_t ret = flowradarsimd_inc_fr(ptr, (const void *)(p_key_bucket + curkick), p_value_bucket[curkick]);
    ...
    uint32_t ret = flowradarsimd_inc_fr(ptr, (const void *)(p_key_bucket + kick_index), p_value_bucket[kick_index]);
...
inline void flowradarsimd_delete(FlowRadarSimdPtr ptr) 
{
    rte_free(ptr->simd_ptr);
    rte_free(ptr);
}

inline uint32_t flowradarsimd_num_searches(FlowRadarSimdPtr ptr)
{ 
...
ModulePtr simdbatch_flowradarsimd_init(ModuleConfigPtr conf) 
{
    ...
    ModuleSimdBatchFlowRadarSimdPtr module = rte_zmalloc_socket(0,
            sizeof(struct ModuleSimdBatchFlowRadarSimd), 64, socket); 

    module->_m.execute = simdbatch_flowradarsimd_execute;
    module->size  = size;
    module->keysize = keysize;
    module->elsize = elsize;
    module->socket = socket;

    module->flowradarsimd_ptr1 = flowradarsimd_create(size, keysize, elsize, socket);
    module->flowradarsimd_ptr2 = flowradarsimd_create(size, keysize, elsize, socket);

    module->flowradarsimd = module->flowradarsimd_ptr1;

    return (ModulePtr)module;
}

void simdbatch_flowradarsimd_delete(ModulePtr module_) 
{
    ModuleSimdBatchFlowRadarSimdPtr module = (ModuleSimdBatchFlowRadarSimdPtr)module_;

    flowradarsimd_delete(module->flowradarsimd_ptr1);
    flowradarsimd_delete(module->flowradarsimd_ptr2);

    rte_free(module);
}

inline void simdbatch_flowradarsimd_execute(
        ModulePtr module_,
        ...
        uint32_t count) 
{
    ...
    FlowRadarSimdPtr flowradarsimd = module->flowradarsimd;

    /* Prefetch flowradarsimd entries */
    for (i = 0; i < count; ++i) 
    {
        uint8_t const* pkt = rte_pktmbuf_mtod(pkts[i], uint8_t const*);
        flowradarsimd_inc(flowradarsimd, (pkt + 26));
    }
}

inline void simdbatch_flowradarsimd_reset(ModulePtr module_) 
{
    ModuleSimdBatchFlowRadarSimdPtr module = (ModuleSimdBatchFlowRadarSimdPtr)module_;
    FlowRadarSimdPtr prev = module->flowradarsimd;

    if (module->flowradarsimd == module->flowradarsimd_ptr1) 
    {
        module->flowradarsimd = module->flowradarsimd_ptr2;
    } 
    else 
    {
        module->flowradarsimd = module->flowradarsimd_ptr1;
    ...
    module->stats_search += flowradarsimd_num_searches(prev);
    flowradarsimd_reset(prev);
}

inline void simdbatch_flowradarsimd_stats(ModulePtr module_, FILE *f) 
{
    ModuleSimdBatchFlowRadarSimdPtr module = (ModuleSimdBatchFlowRadarSimdPtr)module_;
    module->stats_search += flowradarsimd_num_searches(module->flowradarsimd);
    fprintf(f, "SimdBatch::FlowRadarSimd::SearchLoad\t%u\n", module->stats_search);
}
