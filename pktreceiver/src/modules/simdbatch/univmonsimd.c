...
#include "univmonsimd.h"



inline void univmonsimd_reset(UnivMonSimdPtr ptr) 
{
    ...
    for(int i = 0; i < SIMD_ROW_NUM; i++)
    {
        ...
        for(int j = 0; j < 16; j++)
        {
            univmonsimd_inc_um(ptr, (const void *)(p_key_bucket + j), *(p_value_bucket + j));
        }
    }
...
UnivMonSimdPtr univmonsimd_create(uint32_t size, uint16_t keysize, uint16_t elsize, int socket) 
{
    /* Size up so that we are a power of two ... no wasted space */
    size = rte_align32pow2(size);

    UnivMonSimdPtr ptr = rte_zmalloc_socket(
        0, 
        sizeof(struct UnivMonSimd), /* Size of elements */
        64, socket);
...
inline int univmonsimd_inc_um(UnivMonSimdPtr ptr, void const * kick_key, uint32_t kick_val)
{
    count_heap_insert(ptr->CH1, *(uint32_t const*)kick_key, kick_val);

    uint32_t h1 = dss_hash_with_seed(kick_key, ptr->keysize, CMS_SH1) & 1;
    if(h1 == 1)
    {
        count_heap_insert(ptr->CH2, *(uint32_t const*)kick_key, kick_val);
        return 0;
    }

    uint32_t h2 = dss_hash_with_seed(kick_key, ptr->keysize, CMS_SH2 + CMS_SH1) & 1;
    if(h2 == 1)
    {
        count_heap_insert(ptr->CH3, *(uint32_t const*)kick_key, kick_val);
        return 0;
    }

    uint32_t h3 = dss_hash_with_seed(kick_key, ptr->keysize, CMS_SH3 + CMS_SH2) & 1;
    if(h3 == 1)
    {
        count_heap_insert(ptr->CH4, *(uint32_t const*)kick_key, kick_val);
        return 0;
    }
    return 1;
}
...
inline int univmonsimd_inc(UnivMonSimdPtr ptr, void const *key) 
{
    ...
    uint32_t ret = univmonsimd_inc_um(ptr, (const void *)(p_key_bucket + curkick), p_value_bucket[curkick]);
    ...
    uint32_t ret = univmonsimd_inc_um(ptr, (const void *)(p_key_bucket + kick_index), p_value_bucket[kick_index]);
...
inline void univmonsimd_delete(UnivMonSimdPtr ptr) 
{
...
inline uint32_t univmonsimd_num_searches(UnivMonSimdPtr ptr)
{ 
...
ModulePtr simdbatch_univmonsimd_init(ModuleConfigPtr conf) 
{
    ...
    ModuleSimdBatchUnivMonSimdPtr module = rte_zmalloc_socket(0,
            sizeof(struct ModuleSimdBatchUnivMonSimd), 64, socket); 

    module->_m.execute = simdbatch_univmonsimd_execute;
    module->size  = size;
    module->keysize = keysize;
    module->elsize = elsize;
    module->socket = socket;

    module->univmonsimd_ptr1 = univmonsimd_create(size, keysize, elsize, socket);
    module->univmonsimd_ptr2 = univmonsimd_create(size, keysize, elsize, socket);

    module->univmonsimd = module->univmonsimd_ptr1;

    return (ModulePtr)module;
}

void simdbatch_univmonsimd_delete(ModulePtr module_) 
{
    ModuleSimdBatchUnivMonSimdPtr module = (ModuleSimdBatchUnivMonSimdPtr)module_;

    univmonsimd_delete(module->univmonsimd_ptr1);
    univmonsimd_delete(module->univmonsimd_ptr2);

    rte_free(module);
}

inline void simdbatch_univmonsimd_execute(
        ModulePtr module_,
        ...
        uint32_t count) 
{
    ...
    UnivMonSimdPtr univmonsimd = module->univmonsimd;

    /* Prefetch univmonsimd entries */
    for (i = 0; i < count; ++i) 
    {
        uint8_t const* pkt = rte_pktmbuf_mtod(pkts[i], uint8_t const*);
        univmonsimd_inc(univmonsimd, (pkt + 26));
    }
}

inline void simdbatch_univmonsimd_reset(ModulePtr module_) 
{
    ModuleSimdBatchUnivMonSimdPtr module = (ModuleSimdBatchUnivMonSimdPtr)module_;
    UnivMonSimdPtr prev = module->univmonsimd;

    if (module->univmonsimd == module->univmonsimd_ptr1) 
    {
        module->univmonsimd = module->univmonsimd_ptr2;
    } 
    else 
    {
        module->univmonsimd = module->univmonsimd_ptr1;
    ...
    module->stats_search += univmonsimd_num_searches(prev);
    univmonsimd_reset(prev);
}

inline void simdbatch_univmonsimd_stats(ModulePtr module_, FILE *f) 
{
    ModuleSimdBatchUnivMonSimdPtr module = (ModuleSimdBatchUnivMonSimdPtr)module_;
    module->stats_search += univmonsimd_num_searches(module->univmonsimd);
    fprintf(f, "SimdBatch::UnivMonSimd::SearchLoad\t%u\n", module->stats_search);
}
