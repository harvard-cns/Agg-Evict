...
_runEaters() {
    ...
    echo "/home/omid/cache-aggressive-app/run-on-cores.sh $eaterMask"
    /home/omid/memory-bandwidth-demo/run-on-cores.sh $eaterMask
...
#           for dist in 1.1; do
#               RunBenchmark "$bashDir/01-hh-hm-pqueue.yaml" "262144" "3" 1 $dist $eaterSize $eaterMask
#               RunBenchmark "$bashDir/01-hh-hm-simple.yaml" "2097152" "3" 1 $dist $eaterSize $eaterMask
#               #RunBenchmark "$bashDir/01-hh-hm-simple.yaml" "262144 524288 1048576 2097152 4194304 8388608" "3" 1 $dist $eaterSize $eaterMask
#               #RunBenchmark "$bashDir/01-hh-hm-linear.yaml" "262144 524288 1048576 2097152 4194304 8388608" "3" 1 $dist $eaterSize $eaterMask
#               RunBenchmark "$bashDir/01-hh-hm-linear.yaml" "2097152" "3" 1 $dist $eaterSize $eaterMask
#               #RunBenchmark "$bashDir/01-hh-hm-cuckoo-local.yaml" "131072 262144 524288 1048576 2097152 4194304" "3" 1 $dist $eaterSize $eaterMask
#               RunBenchmark "$bashDir/01-hh-hm-cuckoo-local.yaml" "1048576" "3" 1 $dist $eaterSize $eaterMask
#               #RunBenchmark "$bashDir/01-hh-hm-cuckoo-bucket.yaml" "131072 262144 524288 1048576 2097152 4194304" "3" 1 $dist $eaterSize $eaterMask
#               RunBenchmark "$bashDir/01-hh-hm-cuckoo-bucket.yaml" "1048576" "3" 1 $dist $eaterSize $eaterMask
#           done
#       done
#   done
...
for eaterSize in 64; do
    for eaterMask in 0x80 0x280 0xA80 0x2A80 0xAA80 0x2AA80 0xAAA80; do
        for dist in 1.1; do
            RunBenchmark "$bashDir/01-hh-hm-simple.yaml" "2097152" "3" 1 $dist $eaterSize $eaterMask
            RunBenchmark "$bashDir/01-hh-hm-linear.yaml" "2097152" "3" 1 $dist $eaterSize $eaterMask
            RunBenchmark "$bashDir/01-hh-hm-linear-ptr.yaml" "2097152" "3" 1 $dist $eaterSize $eaterMask
            RunBenchmark "$bashDir/01-hh-hm-cuckoo-bucket.yaml" "1048576" "3" 1 $dist $eaterSize $eaterMask
            RunBenchmark "$bashDir/01-hh-hm-cuckoo-local.yaml" "1048576" "3" 1 $dist $eaterSize $eaterMask
            RunBenchmark "$bashDir/01-hh-hm-cuckoo-ptr.yaml" "1048576" "3" 1 $dist $eaterSize $eaterMask
            RunBenchmark "$bashDir/01-hh-hm-pqueue.yaml" "262144" "3" 1 $dist $eaterSize $eaterMask
