    ...
    for benchmark in $benchmarkList; do
        for elsize in $elsizeList; do
            for size in $sizeList; do
                ...
                echo "ssh omid@mm -- ~/measure-pkt-d1-to-d1/run.sh $trafficDist $speed 272630000 >/dev/null 2>&1"
                ssh omid@mm -- ~/measure-pkt-d1-to-d1/run.sh $trafficDist $speed 272630000 >/dev/null 2>&1
...
for dist in def; do
    RunBenchmark "$bashDir/01-hh-hm-pqueue.yaml" "16384 32768 65536 131072 262144 524288 1048576 2097152" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-simple.yaml" "4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152 4194304 8388608" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-linear.yaml" "131072 262144 524288 1048576 2097152 4194304 8388608" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-countmin.yaml" "128 256 512 1024 2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152 4194304 8388608" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-cuckoo-local.yaml" "131072 262144 524288 1048576 2097152 4194304" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-cuckoo-bucket.yaml" "131072 262144 524288 1048576 2097152 4194304" "3" 1 $dist
done

# Experiment to show the accuracy of the simple-count-array -- it's pretty accurate
for dist in 0.5 1.1 1.5 def; do
    RunBenchmark "$bashDir/01-hh-hm-simple.yaml" "2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152 4194304 8388608" "3" 1 $dist
done

# Experiment to show that the distribution effect on the latency
for dist in 0.75 1.1 1.25 1.5 1.75; do
    RunBenchmark "$bashDir/01-hh-hm-linear.yaml" "1048576" "3"  1 $dist
    RunBenchmark "$bashDir/01-hh-hm-simple.yaml" "1048576" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-countmin.yaml" "524288" "3" 1 $dist
    RunBenchmark "$bashDir/01-hh-hm-pqueue.yaml" "262144" "3" 1 $dist
done
...
for dist in 0.75 1.1 1.25 1.5 1.75; do
    for key in 1 3 6 9 12 15; do
        ...
        RunBenchmark "$bashDir/01-hh-hm-linear-ptr.yaml" "1048576" "$key"  1 $dist
    done

    for key in 1 3 6 9 12 15; do
        RunBenchmark "$bashDir/01-hh-hm-cuckoo.yaml" "524288" "$key"  1 $dist
        RunBenchmark "$bashDir/01-hh-hm-cuckoo-ptr.yaml" "524288" "$key"  1 $dist
        RunBenchmark "$bashDir/01-hh-hm-cuckoo-bucket.yaml" "524288" "$key"  1 $dist
    done
done
